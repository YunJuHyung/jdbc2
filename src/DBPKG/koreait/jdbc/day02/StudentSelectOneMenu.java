...
public class StudentSelectOneMenu {
	...
	private static void  selectOneStudent(Connection conn) {
			...
			try (
				...
				//sql 실행하고 `select는 조회 데이터를 결과로 받아 자바 변수에 저장`해야 합니다
				...
				//조회 결과 유무를 알려면 `제일 먼저 실행해야 할 메소드-조회 겨로가 첫번째 행으로 이동` 입니다.
				System.out.println("조회 결과가 있을까요 ?"  + rs.next());
				//조화된 rs 에서 특정 컬럼값을 가져오기 할때,컬럼의 데이터 타입을 확인하고
				...
				//결과 집합을 모두 소모했음 -> 조회 결과가 없는데 rs.getXXXX 메소드 실행할 때 예외 메시지.
			}
			sc.close();
	}
